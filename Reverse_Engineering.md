...
[cite_start]For a homeostatic state vector $\vec{H}(t) = [\text{hunger}(t), \text{safety}(t), \text{border\_stress}(t)]^T$[cite: 13, 14], the macroscopic transition is formalized as:
...
[cite_start]Where $\mathbb{I}$ is the indicator function of the creature's spatial position [cite: 15][cite_start], and $d_{\text{wall}}$ is the minimum distance to the outer boundaries[cite: 15].
...
[cite_start]For a competitive network of $N$ modules with continuous activation drives under a Gaussian stochastic perturbation $\varepsilon_i(t) \sim \mathcal{N}(0, \sigma^2)$[cite: 24, 25], the selection of the unique executing circuit $k$ is defined by:
...
[cite_start]Where $\mathbf{S}_s$ represents the effective homeostatic modulators derived from the hysteresis filter (e.g., `effective_hunger` or `current_safety_need`) [cite: 27][cite_start], and $\mathbf{E}_k$ is a standard basis vector indicating a discrete state activation[cite: 27].
