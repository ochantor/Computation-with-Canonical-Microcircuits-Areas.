...
For a homeostatic state vector $\vec{H}(t) = [\text{hunger}(t), \text{safety}(t), \text{borderstress}(t)]^T$, the macroscopic transition is formalized as:
...
Where $\mathbf{S}_s$ represents the effective homeostatic modulators derived from the hysteresis filter, and $\mathbf{E}_k$ is a standard basis vector indicating a discrete state activation.
